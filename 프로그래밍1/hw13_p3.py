...
class Table :
    def __init__(self) :
        ...
        self.dealer_y = 80 # 딜러 카드의 초기 y위치                                    #고쳐야 할 부분
        self.player_y = 200 # 플레이어 카드의 초기 y 위치                              # 고쳐야 할 부분
        ...
        self.message = Text() # 수정 부분 여기 만들자
        self.question = Text() # 수정 부분  여기 만들자
...
def players_turn(deck, player, table) :
    while player.hand_value() < 21 : # 아직 21을 넘지 않아 게임이 진행 가능한 경우
        if not take_response("Would you like another card? (y/n)") : # 카드를 계속 뽑을지 묻는 조건문 -- table.ask_response()로 수정
            break
        # 카드를 계속 뽑는 경우
...
def conclude_the_game(player, dealer) :
    ...
    if player_total > 21 : # 플레이어 패배 경우
        return
    if dealer_total > 21 : # 딜러의 패배 경우
        print("The dealer went over 21! You won!")
        return
    print("\nYour Total was", player_total)
    print("The dealer's total was", dealer_total)
    # 둘다 21안에 있을 때 대소 비교
