...
def max_val(row, index,val, memo, a=0, b=0, c=0, d=0, result1=0, result2=0, result3=0, result4=0) :
    ...
    if a > 0 :
        memo[(row, index)] = a
        result1 = max_val(row, index-1, mountain[row][index-1], memo) + a
        #memo[(row, index)] = result1

    if b > 0 :
        memo[(row, index)] = b
        result2 = max_val(row, index+1,  mountain[row][index+1], memo) + b
        #memo[(row, index)] = result2

    if c > 0 :
        memo[(row, index)] = c
        result3 = max_val(row-1, index, mountain[row-1][index], memo) + c
        #memo[(row, index)] = result3

    if d > 0 :
        memo[(row, index)] = d
        result4 = max_val(row+1, index, mountain[row+1][index], memo) + d
        #memo[(row, index)] = result1
...
for row in mountain :
    for val in mountain[row] :
        ...
        accum = max_val(row, index, val, memo)
        Max = max(accum, Max)
