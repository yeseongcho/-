...
result3 = 0
result4 = 0
def max_edge(row,index,val,results, memo, a=0, b=0, c=0, d=0, result1=0, result2=0, result3=0, result4=0) :
    # 메모리에 있는 경우
    if (row, index) in memo :
        ...
        return max_edge(row,index,val,results, memo)
    ...
    if max(a, b, c, d) == a :
        ...
        results = a + results
        #results = a
        result1 = max_edge(row, index-1, mountain[row][index-1], results, memo)
        results = 0
    if max(a, b, c, d) == b :
        memo[(row, index)] = (b, (row, index+1))
        #results = b
        results = b + results
        result2 = max_edge(row, index+1, mountain[row][index+1], results, memo)
        results = 0
    if max(a, b, c, d) == c :
        memo[(row, index)] = (c, (row-1, index))
        #results = c
        results = c + results 
        result3 = max_edge(row-1, index, mountain[row-1][index], results, memo)
        results = 0
    if max(a, b, c, d) == d :
        memo[(row, index)] = (d, (row+1, index))
        #results = d
        results = d + results
        result4 = max_edge(row+1, index, mountain[row+1][index], results, memo)
        results = 0
...
for row in mountain :
    for val in mountain[row] :
        ...
        results = 0
        accum = max_edge(row,index,val,results, memo)
        #print(accum)
        #print(memo)
        Max = max(accum, Max)
print(Max)
