...
def make_short(f_func, g_func, p, q, short_f_x, short_f_y, short_g_x, short_g_y) :
        ...
        if len(f_func) == 1 :
            ...
            for s in range(len(f_func)) :
                if f_func[s][0] <= p and f_func[s+1][0] > p :
                    ...
                    break
        #print(m)
            for i in range(m, len(f_func)) :
                if f_func[i][0] > q :
                    break
                if f_func[i][0] >= p and f_func[i][0] <= q :
                    if f_func[i][0] not in short_f_x :
                        short_f_x.append(f_func[i][0])
                    if f_func[i][1] not in short_f_y :
                        short_f_y.append(f_func[i][1])
    ...
    if g_func[0][0] == q :
        ...
        if len(g_func) == 1 :
            ...
            for k in range(len(g_func)) :
                if g_func[k][0] <= p and g_func[k+1][0] > p :
                    short_g_x.append(p)
                    short_g_y.append(g_func[k][1])
                    n = k
                    break
        #print(short_g_y)
            for j in range(n, len(g_func)) :
                if g_func[j][0] > q :
                    break
