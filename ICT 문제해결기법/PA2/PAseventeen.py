...
def find_maxindex2(people_list, dicts) :
    sorting_people = quick_sort(people_list)
    c = sorting_people[len(sorting_people)-1] + people_list[len(sorting_people)-2]
    r = len(sorting_people)-3
    while( c >= sorting_people[r]) :
        c = c + sorting_people[r]
        r = r - 1
        if r == 0 :
            return r, False
    #print(r)
    ...
    if((Logic == True) and (Logic2 == False)):
        for j in range(index, 0, -1) :
            ...
            if cum > Max :
                Max = cum
            if o < index + 1 :
                if people_list[o] == people_list[k] :
                    break
            #if people_list[o] == people_list[k] :
             #   break
        ...
        for j in range(0, index) :
            ...
            if cum > Max :
                Max = cum
            if m > -1 :
                if people_list[m] == people_list[r] :
                    break
            #if people_list[m] == people_list[r] :
               # break
    else :
