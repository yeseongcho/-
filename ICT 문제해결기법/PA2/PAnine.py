...
def find_maxindex(people_list, dicts) :
    ...
    return k, True
"""
def find_nextindex(people_list, dicts) :
    sorting_people = quick_sort(people_list)
    d = sorting_people[0] + sorting_people[1]
    v = 3
    while(d > sorting_people[v]) :
            d = d + sorting_people[k]
            v = v+1
            if v ==len(sorting_people) -1 :
                return v, False
    ...
    if Logic == False :
        for j in range(index, 0, -1) :
            ...
            if cum > Max :
                Max = cum
            if m < index - 2 :
                if people_list[m+2] > Cum(sorting_people, m+1) :
                    break
...
def main() :
    ...
    #v, Logic2 = find_nextindex(people_list, dicts)
