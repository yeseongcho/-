...
# 같은 값을 발견한 뒤 더 큰 값이 있는지 linear search..! -- 이 곳을 다시 binary search하는 거 같아..!
while (distance + 1 != th) :
    if count > k :
        now = 0
        compare = 1
        ...
        compare = 1
        count = 1
        distance = len_search(correct+1, distance-1, th)
        if distance == th-2 :
            break
        count = get_count(now, compare, count)
    else :
        ...
        distance_max = max(distance_max, distance)
        correct = distance_max
        distance = len_search(correct+1, th, th)
        if distance == th-2 :
            break
        count = get_count(now, compare, count)
